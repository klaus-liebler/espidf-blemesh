...
RGBLED::M<1, RGBLED::DeviceType::WS2812>* strip{nullptr};
CRGB colors[]{CRGB::Red, CRGB::Green, CRGB::Blue, CRGB::White};

void board_led_operation(uint8_t ledIndex, bool onoff)
{
    ESP_LOGI(TAG, "Set ledIndex %d to %d", ledIndex, onoff);
    if (strip->SetPixel(ledIndex, onoff ? colors[ledIndex] : CRGB::Black, true) != ErrorCode::OK)
    {
...
extern "C" void app_main(void)
{
    ...
    strip = new RGBLED::M<1, RGBLED::DeviceType::WS2812>();
    ERRORCODE_CHECK(strip->Begin(SPI3_HOST, GPIO_NUM_48));
    ERRORCODE_CHECK(strip->Clear(100));
    strip->SetPixel(0, CRGB::Red, true);
    ESP_ERROR_CHECK(nvs_flash_init());
    /* Open nvs namespace for storing/restoring mesh example info */
    ESP_ERROR_CHECK(ble_mesh_nvs_open(&NVS_HANDLE));
    ...
    ESP_ERROR_CHECK(esp_ble_mesh_set_unprovisioned_device_name("LiebKnobs"));
    button_handle_t btn_handle = iot_button_create(GPIO_NUM_0, BUTTON_ACTIVE_LOW);
    if (btn_handle) {
        iot_button_set_evt_cb(btn_handle, BUTTON_CB_RELEASE, button_tap_cb, (void*)"RELEASE");
    }
