...
RGBLED::M<4, RGBLED::DeviceType::WS2812>* strip{nullptr};
...
void board_led_operation(uint8_t ledIndex, bool onoff)
{
    ...
    if (strip->SetPixel(4 - ledIndex - 1, onoff ? colors[ledIndex] : CRGB::Black, true) != ErrorCode::OK)
    {
                                     ...
                                     esp_ble_mesh_msg_ctx_t *ctx, uint8_t onoff)
{
    ...
    else if (ESP_BLE_MESH_ADDR_IS_GROUP(ctx->recv_dst))
    {
        ...
        {
            led = model->element->element_addr - primary_addr;
            board_led_operation(led, onoff);
        }
    }
...
extern "C" void app_main(void)
{
    ...
    strip = new RGBLED::M<4, RGBLED::DeviceType::WS2812>();
    ERRORCODE_CHECK(strip->Begin(SPI3_HOST, GPIO_NUM_13));
    ...
    ESP_ERROR_CHECK(esp_ble_mesh_set_unprovisioned_device_name("LiebLights"));
}
